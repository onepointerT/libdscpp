...
/*-------------------------------------------------------------------------*/
/**
  @brief    Default error callback for dictionary: wraps `fprintf(stderr, ...)`.
 */
/*--------------------------------------------------------------------------*/
...
static int (*dictionary_error_callback)(const char*, ...) = default_error_callback;
...
{
  if (errback) {
    dictionary_error_callback = errback;
  } else {
    dictionary_error_callback = default_error_callback;
  }
}
...
void dictionary_dump_ini(const dictionary * d, FILE * f)
{
    ...
    for (i=0 ; i<nsec ; i++) {
        secname = dictionary_getsecname(d, i) ;
        dictionary_dumpsection_ini(d, secname, f);
    }
    fprintf(f, "\n");
    return ;
}
...
void dictionary_dumpsection_ini(const dictionary * d, const char * s, FILE * f)
{
    ...
    if (d==NULL || f==NULL) return;
    if (! dictionary_find_entry(d, s)) return;
...
int dictionary_getsecnkeys(const dictionary * d, const char * s)
{
    ...
    if (! dictionary_find_entry(d, s)) return nkeys;
...
const char ** dictionary_getseckeys(const dictionary * d, const char * s, const char ** keys)
{
    ...
    if (! dictionary_find_entry(d, s)) return NULL;
...
long int dictionary_getlongint(const dictionary * d, const char * key, long int notfound)
{
    ...
    str = dictionary_getstring(d, key, INI_INVALID_KEY);
...
{
    return (int)dictionary_getlongint(d, key, notfound);
}
...
int dictionary_getboolean(const dictionary * d, const char * key, int notfound)
{
    ...
    c = dictionary_getstring(d, key, INI_INVALID_KEY);
...
const char* dictionary_key_getsec(const char * key) {
    const size_t dppos = strfindpossimple( key, ":" );
    if ( dppos >= strlen(key) ) return "";
    return strsubstr( key, 0, dppos-1 );
}
...
const char* dictionary_key_getkey(const char * key) {
    const size_t dppos = strfindpossimple( key, ":" );
    if ( dppos >= strlen(key) ) return "";
    return strsubstr( key, dppos+1, strlen(key)-1 );
}
...
const size_t dictionary_key_getnpos(const dictionary * d, const char * key) {
    for ( unsigned int di = 0; di < d->n; di++ ) {
        if ( strcmp(d->key[di], key) == 0 ) return di;
    }
...
{
    int found=0 ;
    if (dictionary_getstring(ini, entry, INI_INVALID_KEY)!=INI_INVALID_KEY) {
        found = 1 ;
    }
    return found ;
}
...
static void parse_quoted_value(char *value, char quote) {
    ...
    if(!quoted) {
        dictionary_error_callback("dictionary: memory allocation failure\n");
        goto end_of_value;
    }
 ...
 */
/*--------------------------------------------------------------------------*/
static line_status dictionary_line(
...
dictionary * dictionary_load_file(FILE * in, const char * ininame)
{
    ...
    while (fgets(line+last, ASCIILINESZ-last, in)!=NULL) {
        ...
        if (line[len]!='\n' && !feof(in)) {
            dictionary_error_callback(
              "dictionary: input line too long in %s (%d)\n",
              ininame,
              lineno);
            dictionary_del(dict);
            return NULL ;
        ...
        switch (dictionary_line(line, section, key, val)) {
            case LINE_EMPTY:
            case LINE_COMMENT:
            break ;
            ...
            case LINE_ERROR:
            dictionary_error_callback(
              "dictionary: syntax error in %s (%d):\n-> %s\n",
              ininame,
              lineno,
              line);
            errs++ ;
...
dictionary * dictionary_load(const char * ininame)
{
    ...
    if ((in=fopen(ininame, "r"))==NULL) {
        dictionary_error_callback("dictionary: cannot open %s\n", ininame);
        return NULL ;
    }

    dict = dictionary_load_file(in, ininame);
    fclose(in);
